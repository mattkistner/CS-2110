...
int stack_push(const char *question, enum category_t category) {
    // TODO
    //Initialize post_t
    struct post_t *newPost = malloc(sizeof(struct post_t));
    if (!newPost) {
        return FAILURE;
    }
    ...
    newPost->question = malloc(strlen(question) + 1);
    if (!(newPost->question)) {
        free(newPost);
        return FAILURE;
    }
...
int stack_pop(struct post_t *data) {
    if (data == NULL || numPosts == 0 || stack_arr == NULL) {
        return FAILURE;
    } else {
        data = *(stack_arr + (numPosts));
        numPosts--;
        return SUCCESS;
    }
    UNUSED(data);
